...
metaPsi$Mean <- apply(metaPsi[, 7:151], 1, mean, na.rm = TRUE)
...
ggplot(retention_df, aes(x = LENGTH, y = Max)) +
  ...
  scale_x_log10() +
  labs(title = "Intron Length vs Max Percent Spliced In (PSI) (Hexbin)",
...
ggplot(data.frame(PSI = valuesPsiVector), aes(x = PSI)) +
  ...
  labs(title = "Distribution of All Percent Spliced In (PSI) Values",
       x = "PSI (%)",
       y = "Count of Intron Retention Events") +
  theme_minimal()
```
...
ggplot(retention_df, aes(x = length_bin, y = Median, fill = length_bin)) +
  ...
  labs(title = "Median Percent Spliced In (PSI) by Intron Length Deciles",
       x = "Intron Length Bin (Decile + Range in bases)",
       y = "Median PSI",
       fill = "Length Bin") +
  theme_minimal() +
...
ggplot(metaPsi, aes(x = Mean)) +
    geom_histogram(binwidth = 4, fill = "steelblue", colour = "black") +
    labs(title = "Distribution of Average Percent Spliced In (PSI) Values",
         x = "Average PSI (%)",
         y = "Count of Intron Retention Events") +
    theme_minimal()
```
...
ggplot(sampleIrDf, aes(y = reorder(Sample, IrEventCount), x = IrEventCount, fill = Group)) +
  ...
  labs(title = "Total Intron Retention Events in Top 30 Tissue Samples (PSI ≥ 15)",
       y = "Tissue Sample",
       x = "Number of Intron Retention Events",
       fill = "Group") +
  scale_fill_manual(values = c("Developmental" = "turquoise", "Mature" = "salmon")) +
  theme_minimal() +
...
sampleIrDf$Group <- ifelse(grepl(paste0("^(", paste(youngPrefixes, collapse = "|"), ")"),
                                 sampleIrDf$Sample),
                           "Developmental", "Mature")
sampleIrDf <- sampleIrDf[order(-sampleIrDf$IrEventCount), ][1:30, ]
