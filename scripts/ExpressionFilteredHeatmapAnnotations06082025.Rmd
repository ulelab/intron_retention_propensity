...
```{r}
# Clean up
rm(expressionClean, expressionCleanLowMasked, countData, cleanPsi)
```
...
metaPsiWithClusters[, c("chr", "start", "end") := tstrsplit(gsub("^chr", "", COORD), "[:-]")]
metaPsiWithClusters[, `:=`(start = as.integer(start), end = as.integer(end))]
metaPsiWithClusters[, strand := substr(FullCO, nchar(FullCO), nchar(FullCO))]

# Convert to GRanges
gr_meta <- GRanges(
  seqnames = metaPsiWithClusters$chr,
  ranges = IRanges(start = metaPsiWithClusters$start, end = metaPsiWithClusters$end),
  strand = metaPsiWithClusters$strand
)
...
# Carry metadata
mcols(gr_meta) <- metaPsiWithClusters
...
```{r}
duplicated_introns <- mergedClusterData$EVENT[duplicated(mergedClusterData$EVENT)]
length(unique(duplicated_introns))  # Number of introns with >1 match
...
psi_cols <- names(mergedDataFiltered)[8:152]
expr_cols <- names(mergedDataFiltered)[158:302]
...
#fwrite(mergedDataFiltered, file = "mergedRPKMData181332introns.tsv", sep = "\t", quote = FALSE)
...
mergedCleanedData <- mergedCleanedData[, -c(1, 2, 4:7, 153:157, 303:308), with = FALSE]
...
#fwrite(mergedCleanedData, file = "mergedCleanedData181332introns.tsv", sep = "\t", quote = FALSE)
...
# Sort annotation objects based on cleaned data events then drop event for heatmap plotting
annotation_row <- annotation_row[mergedCleanedData$EVENT, ]
rownames(mergedCleanedData) <- mergedCleanedData$EVENT
mergedCleanedData <- mergedCleanedData %>% select(-EVENT)
mergedCleanedMatrix <- as.matrix(mergedCleanedData)

# Always carry through rownames or annotations will error
rownames(mergedCleanedMatrix) <- rownames(mergedCleanedData)
...
```{r}
png("IRHeatmap08082025.png", width = 1600, height = 1600, res = 150)
