...
expressionTable <- fread("~/Documents/Career/Georges/Ule/intron_retention_propensity/data/VastDB/EXPRESSION_TABLE-hg38.tab")
psiTable <- fread("~/Documents/Career/Georges/Ule/intron_retention_propensity/data/VastDB/PSI_TABLE-hg38.tab")
```
...
sampleGroups <- fread("~/Documents/Career/Georges/Ule/intron_retention_propensity/data/VastDB/vastdb_Sample_Groups.csv")
...
plotListAll <- lapply(df, function(tissue) {
  psi_col <- paste0(tissue, "_PSI")
  expr_col <- paste0(tissue, "-cRPKM")
  dt <- mergedClusteredData[, .(Tissue = tissue,
                                PSI = get(psi_col),
...
plot_cluster_hexbin <- function(cluster_id, bins = 30) {
  ...
  p <- ggplot(cluster_data, aes(x = cRPKM, y = PSI)) +
    ...
    scale_x_log10() +
    labs(title = paste("PSI vs cRPKM Across Tissue Groups –", paste("Cluster", cluster_id)),
         x = "Expression (cRPKM, log scale)",
         y = "PSI") +
    theme_minimal() +
    theme(strip.text = element_text(size = 9),
...
df <- fread("/Users/mikej/Documents/Career/Georges/Ule/intron_retention_propensity/data/Ir/mergedwithtransposon_final06072025.tsv")

psi_cols <- grep("_PSI$", names(df), value = TRUE)
expr_cols <- sub("_PSI$", "-cRPKM", psi_cols)

plotDataAll <- rbindlist(lapply(seq_along(psi_cols), function(i) {
    data.table(
        PSI = df[[psi_cols[i]]],
        cRPKM = df[[expr_cols[i]]],
        CLUSTER = df$CLUSTER
    )
}))
