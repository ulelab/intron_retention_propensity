...
mergedPSI <- fread("/Users/mikej/Documents/Career/Georges/Ule/intron_retention_propensity/data/VastDB/mergedMinorAnno192951introns.tsv")
```
...
```{r}
# Look for specific intron categories
...
cols_to_move <- colnames(psi_trans_wide)[301:362]

# Capture the names of the first 11 columns
first_block <- colnames(psi_trans_wide)[1:10]
...
# Reorder: first 10 cols → cols to move → remaining
psi_trans_wide <- psi_trans_wide %>%
  select(all_of(first_block), all_of(cols_to_move), all_of(remaining_cols))

psi_trans_wide <- psi_trans_wide[, -c(15, 16, 60:63)]

colnames(psi_trans_wide)[2] <- "fivePrimeSSPRPF8PredictionSum"
colnames(psi_trans_wide)[3] <- "midIntronPRPF8PredictionSum"
colnames(psi_trans_wide)[4] <- "threePrimeSSPRPF8PredictionSum"
colnames(psi_trans_wide)[11] <- "strand"
```
...
write.table(
  psi_trans_wide,
  file = "mergedTransposons192951introns.tsv",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
)
