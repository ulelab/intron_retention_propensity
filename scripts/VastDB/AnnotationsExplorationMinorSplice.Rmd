...
mergedPSI <- fread("/Users/mikej/Documents/Career/Georges/Ule/intron_retention_propensity/data/VastDB/mergedPRPF8sums192951introns.tsv")
```
```{r}
...
# extract chr start end and strand from PSI table
mergedPSI$strand <- substr(mergedPSI$FullCO, nchar(mergedPSI$FullCO), nchar(mergedPSI$FullCO))
mergedPSI$chrom <- sub(":.*", "", mergedPSI$COORD)
mergedPSI$start <- sub(".*:(\\d+)-\\d+", "\\1", mergedPSI$COORD)
mergedPSI$end   <- sub(".*-(\\d+)", "\\1", mergedPSI$COORD)

# coerce start and end to numeric vectors
mergedPSI$start <- suppressWarnings(as.integer(mergedPSI$start))
mergedPSI$end   <- suppressWarnings(as.integer(mergedPSI$end))

# extract chr start end and strand from minor introns
...
minorintronAnno$strand <- str_extract(minorintronAnno$intron_name, "(?<=_)\\w$")
minorintronAnno$strand <- ifelse(minorintronAnno$strand == "F", "+", "-")
...
```{r}
# Find overlaps
idx <- nearest(gr_minor, gr_psi)  # integer vector: for each query, index of nearest subject (or NA)

matched <- tibble::tibble(
  minor_index = which(!is.na(idx)),
  psi_index   = idx[!is.na(idx)]
) %>%
  dplyr::mutate(
...
matched <- matched %>%
  dplyr::bind_cols(minorintronAnno[matched$minor_index, ])
...
mergedPSI_with_minor <- mergedPSI_with_minor %>% mutate(splice_length = intron_end - intron_start)
...
```{r}
write.table(mergedPSI_best_match, file = "mergedMinorAnno192951introns.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
```
