...
# Create a Max column for quality control analysis, 24,716 Events did not have a Max value in Event Metrics
...
# K-means clustering (k = 5) on imputed matrix
set.seed(43)
kmeansRes <- kmeans(imputeMatrix, centers = 5)
clusterLabels <- data.frame(cluster = factor(paste0("Cluster_", kmeansRes$cluster),
                                             levels = paste0("Cluster_", 1:5)))
...
tissueAnnotation <- data.frame(Tissue = str_extract(colnames(imputeMatrix), "^[^_]+"))
rownames(tissueAnnotation) <- colnames(imputeMatrix)

# Compute mean PSI per tissue per cluster
imputeLong <- as.data.frame(imputeMatrix) %>%
  mutate(Cluster = clusterLabels[rownames(imputeMatrix), "cluster"]) %>%
  pivot_longer(-Cluster, names_to = "Sample", values_to = "PSI") %>%
  mutate(Tissue = str_extract(Sample, "^[^_]+"))

tissueClusterSummary <- imputeLong %>%
  group_by(Cluster, Tissue) %>%
  summarise(MeanPSI = mean(PSI, na.rm = TRUE), .groups = "drop")

# Get top 3 tissues per cluster and filter annotation
topTissues <- tissueClusterSummary %>%
  group_by(Cluster) %>%
  slice_max(order_by = MeanPSI, n = 3)

topTissueSet <- unique(topTissues$Tissue)
tissueAnnotation$Tissue[!tissueAnnotation$Tissue %in% topTissueSet] <- NA

# Plot heatmap

colorScale <- colorRampPalette(c("lightblue", "dodgerblue", "pink", "firebrick"))(100)
png("IRClusters5kmeansHeatmap12052025.png", width = 1600, height = 1200, res = 150)
pheatmap(matrixOrdered,
         ...
         color = colorScale,
         na_col = "grey90",
         main = "Intron Retention Clusters with Tissue Context")
dev.off()
...
youngPrefixes <- c("Amnion", "Embr", "ESC", "iPS", "MSC", "NCC", "NPC", "Oocyte", 
                   "Pancreas_Alpha_Young", "Pancreas_Beta_Young", "Placenta", 
                   "Young", "Zygote")
